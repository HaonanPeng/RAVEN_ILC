...
tf::Transform Raven_PathPlanner::ComputeLineTrajectory()
{
	...
	del_Vector = del_Vector.normalized()*Speed*Li_Direction;

	if (((del_Vector.length()) > Li_MaxSp) || ((del_Vector.length()) < -1 * Li_MaxSp))
	{
		ROS_ERROR("Speed is too high, sending NULL Trajectory");
		del_Vector.setValue(0, 0, 0);
	}
